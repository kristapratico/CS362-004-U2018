...
int adventurerEff(struct gameState *state){
  ...
  //adventurer effect extracted from cardEffect
...
int smithyEff(struct gameState *state, int handPos){
  int currentPlayer = whoseTurn(state);
  int i; //for for loop
...
int greatHallEff(struct gameState *state, int handPos){
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case smithy:
      return smithyEff(state, handPos);
		
    case village:
      return villageEff(state, handPos);
    ...
    case great_hall:
      return greatHallEff(state, handPos);
